...
class PatchPaginatedResponseDto extends AbstractDto
{
    /**
    ...
    public function getData(): array
    {
        ...
        foreach ((array) ($this->data['data'] ?? []) as $item) {
            if (is_array($item)) {
                $out[] = PatchResponseDto::fromArray($item);
            } elseif ($item instanceof PatchResponseDto) {
                $out[] = $item;
            }
        }
        return $out;
